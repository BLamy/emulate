...
export const SERVICE_REGISTRY: Record<ServiceName, ServiceEntry> = {
  ...
  auth0: {
    ...
    endpoints:
      "OIDC discovery, JWKS, OAuth token (client_credentials, password-realm, refresh_token), userinfo, revoke, Management API users, users-by-email, tickets, log streams",
    async load() {

...
The Auth0 emulator supports OIDC discovery, RS256 ID tokens, password-realm grants, refresh token grants, and client credentials grants.

```bash
curl http://localhost:4007/.well-known/openid-configuration
```

Management API endpoints require a Bearer token obtained with the `client_credentials` grant. The token endpoint accepts `client_id` and `client_secret` in the request body or as a Basic auth header.

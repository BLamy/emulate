...
- `AttachUserPolicy` / `DetachUserPolicy` / `ListAttachedUserPolicies` - user managed policy attachments
- `AttachRolePolicy` / `DetachRolePolicy` / `ListAttachedRolePolicies` - role managed policy attachments
...
- `AssumeRole` - assume a role. Returns temporary credentials with session tag metadata. `DurationSeconds` must be from 900 to 43200 seconds.

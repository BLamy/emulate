...
- `CreateApi` / `GetApi` / `GetApis` / `DeleteApi` - HTTP API metadata
- `CreateIntegration` / `GetIntegration` / `GetIntegrations` / `DeleteIntegration` - `AWS_PROXY` Lambda integrations
- `CreateRoute` / `GetRoute` / `GetRoutes` / `DeleteRoute` - exact HTTP routes, `ANY` routes, greedy proxy routes, and `$default`
- `CreateStage` / `GetStage` / `GetStages` / `DeleteStage` - local stages, including `$default`
- `GET`, `POST`, `PUT`, `PATCH`, `DELETE` under `/_aws/apigatewayv2/<api-id>/...` - local Lambda proxy route invokes

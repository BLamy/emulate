...
`signing_key` can provide deterministic `private_key_pem`, `public_key_pem`, and optional `kid` values for static JWT validation.
...
Default seed (always created): an empty JSON stream at `/streams/emulate-default`.

...
| --------- | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `service` | _(required)_ | Service name: `'vercel'`, `'github'`, `'google'`, `'slack'`, `'apple'`, `'microsoft'`, `'okta'`, `'auth0'`, `'aws'`, `'durable-streams'`, `'resend'`, `'stripe'`, `'mongoatlas'`, `'clerk'`, or `'linear'`                                                                                           |
...
Auth0 Authentication API, Management API v2, OpenID Connect, and log stream emulation.
...
- `PATCH /api/v2/users/:id` - update user metadata, profile fields, password, and verification state
...
Dispatches Auth0 log events (`ss`, `fs`, `sv`, `scp`) via webhook on user creation, signup failure, email verification, and password changes. Configure subscribers with `log_streams` in the seed config.
...
Durable Streams protocol emulation for local append-only streams. Routes are method-based and work with the `@durable-streams/client` package when pointed at the emulator URL.

- `PUT /streams/:path` - create a stream, optionally with initial body data
- `HEAD /streams/:path` - read stream metadata
- `GET /streams/:path?offset=-1` - read from an offset
...
The emulator supports JSON-mode array formatting, `Stream-Seq`, idempotent producer headers, `Stream-TTL`, `Stream-Expires-At`, basic fork creation, and `GET /_inspector`.
...
packages/
  ...
  @emulators/
    ...
    durable-streams/ # Durable Streams protocol
apps/
